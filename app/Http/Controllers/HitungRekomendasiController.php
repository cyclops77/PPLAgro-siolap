...
class HitungRekomendasiController extends Controller
{
        ...
        if (strcasecmp($kota->item(24)->nodeValue, "jember") == 0) {
            ...
            foreach ($data as $value) {
                ...
                if ($response["cuaca"]=="Cerah") {
                	...
                	$response["CountCuacaAngka1"] = 1;
                }else if ($response["cuaca"]=="Hujan Sedang") {
                	$response["cuacaAngka"] = 500;
                	$response["CountCuacaAngka"] = 1;
                }
                ...
                if ($response["cuaca2"]=="Cerah") {
                	...
                	$response["CountCuacaAngka2"] = 1;
                }else if ($response["cuaca2"]=="Hujan Sedang") {
                	$response["cuacaAngka2"] = 500;
                	$response["CountCuacaAngka2"] = 1;
                ...
                if ($response["cuaca3"]=="Cerah") {
                	...
                	$response["CountCuacaAngka3"] = 1;
                }else if ($response["cuaca3"]=="Hujan Sedang") {
                	$response["cuacaAngka3"] = 500;
                	$response["CountCuacaAngka3"] = 1;
                ...
                if ($response["cuaca4"]=="Cerah") {
                	...
                	$response["CountCuacaAngka4"] = 1;
                }else if ($response["cuaca4"]=="Hujan Sedang") {
                	$response["cuacaAngka4"] = 500;
                	$response["CountCuacaAngka4"] = 1;
...
//==============================================///
            ...
            foreach ($data as $value) {
                ...
                if ($response["cuaca"]=="Cerah") {
                	...
                	$response["cuacaAngka1"] = 500;
                	$response["CountCuacaAngka1"] = 1;
                }else if ($response["cuaca1"]=="Hujan Sedang") {
                	$response["cuacaAngka1"] = 500;
                	$response["CountCuacaAngka1"] = 1;
                }
        ...
        return view('rekomendasi.perhitungan', compact('response'));
    }
    ...
    public function perhitungan(Request $req)
    {
    	...
    	$normalisasiCuaca = 0.3;
    	$normalisasiTanah = 0.4;


    	if ($req->tanah=="andosol") {
    		$paramTanah = 1;
    	}else if ($req->tanah=="aluvial") {
    		$paramTanah = 2;
    	}

    	if ($req->komoditas=="padi") {
    		
    		$utilitySuhu = ($cmax-$req->paramSuhu)/($cmax-$cmin);
    		$utilityKelembaban = ($cmax-$req->paramKelembaban)/($cmax-$cmin);
    		$utilityCuaca = ($cmax-$req->paramCuaca)/($cmax-$cmin);
    		$utilityTanah = ($cmaxTanah-$paramTanah)/($cmaxTanah-$cmin);

    		$hasilAkhir = (($utilitySuhu*$normalisasiSuhu) + ($utilityKelembaban*$normalisasiKelembaban) + ($utilityCuaca*$normalisasiCuaca) + ($utilityTanah*$normalisasiTanah)*100);

    	}else if ($req->komoditas=="jagung") {
    		
    		$utilitySuhu = ($cmax-$req->paramSuhu2)/($cmax-$cmin);
    		$utilityKelembaban = ($cmax-$req->paramKelembaban2)/($cmax-$cmin);
    		$utilityCuaca = ($cmax-$req->paramCuaca2)/($cmax-$cmin);
    		$utilityTanah = ($cmaxTanah-$paramTanah)/($cmaxTanah-$cmin);

    		$hasilAkhir = (($utilitySuhu*$normalisasiSuhu) + ($utilityKelembaban*$normalisasiKelembaban) + ($utilityCuaca*$normalisasiCuaca) + ($utilityTanah*$normalisasiTanah)*100);

    	}else if ($req->komoditas=="kedelai") {
    		
    		$utilitySuhu = ($cmax-$req->paramSuhu3)/($cmax-$cmin);
    		$utilityKelembaban = ($cmax-$req->paramKelembaban3)/($cmax-$cmin);
    		$utilityCuaca = ($cmax-$req->paramCuaca3)/($cmax-$cmin);
    		$utilityTanah = ($cmaxTanah-$paramTanah)/($cmaxTanah-$cmin);

    		$hasilAkhir = (($utilitySuhu*$normalisasiSuhu) + ($utilityKelembaban*$normalisasiKelembaban) + ($utilityCuaca*$normalisasiCuaca) + ($utilityTanah*$normalisasiTanah)*100);

    	}else if ($req->komoditas=="tembakau") {
    		
    		$utilitySuhu = ($cmax-$req->paramSuhu4)/($cmax-$cmin);
    		$utilityKelembaban = ($cmax-$req->paramKelembaban4)/($cmax-$cmin);
    		$utilityCuaca = ($cmax-$req->paramCuaca4)/($cmax-$cmin);
    		$utilityTanah = ($cmaxTanah-$paramTanah)/($cmaxTanah-$cmin);

    		$hasilAkhir = (($utilitySuhu*$normalisasiSuhu) + ($utilityKelembaban*$normalisasiKelembaban) + ($utilityCuaca*$normalisasiCuaca) + ($utilityTanah*$normalisasiTanah)*100);

    	}else if ($req->komoditas=="tebu") {
    		
    		$utilitySuhu = ($cmax-$req->paramSuhu5)/($cmax-$cmin);
    		$utilityKelembaban = ($cmax-$req->paramKelembaban5)/($cmax-$cmin);
    		$utilityCuaca = ($cmax-$req->paramCuaca5)/($cmax-$cmin);
    		$utilityTanah = ($cmaxTanah-$paramTanah)/($cmaxTanah-$cmin);
